...
def generate_timetable(data):
    ...
    def pick_subject():
        r = random.random() # creates a random decimal number between 0 and 1
        if r < 0.5: # 50% chance to pick from weak subjects
            return random.choice(weak)
        elif r < 0.8: #30% normal b/w 0.5 and 0.8
            return random.choice(normal)
        return random.choice(strong) # 20% strong
    ...
    for d in range(1, days + 1):
        ...
        while current < end_day:
            ...
            if (total_study_done >= study_minutes_goal and total_break_done >= break_minutes_goal):
                break

            # Meals
            for name, (s, e) in meals.items():
                if s and not meal_done[name] and s <= current < e:
                    output.append(f"{name}: {format_time(current)} - {format_time(e)}")
                    current = e
                    meal_done[name] = True
            ...
            if total_study_done < study_minutes_goal:
                ...
                duration = max(25, min(duration, study_minutes_goal - total_study_done)) # min 25 min = default 

                output.append(f"{subject.title()}: {format_time(current)} - {format_time(current+duration)}")
                ...
                if session_count >= break_after:
                    b = min(base_break, break_minutes_goal - total_break_done)
                    output.append(f"Break: {format_time(current)} - {format_time(current+b)}")
                    current += b
                    total_break_done += b
                    session_count = 0
